 ...
 */
package org.aiotml.testing.reportbuilder;
...
import org.aiotml.testing.reportbuilder.parser.Suite;
import org.aiotml.testing.reportbuilder.parser.Test;
...
public class ReportBuilder {
	...
	private void buildDocument(Element head, Element body) {
		// Set title
		head.addElement("title").addText("AiotML :: Testing :: Report");
	...
	private void buildTestsSummary(Element parent) {
		...
		filterRow.addElement("div").addAttribute("class", "col-sm-2")
		         .addElement("button").addAttribute("id", "aiotml-only-failures").addAttribute("type", "button").addAttribute("class", "btn btn-default")
	...
	private void buildSuites(Element parent, List<Suite> suites) {
		for (Suite suite : suites) {
			...
			Element div = parent.addElement("div").addAttribute("class", "aiotml-suite"+hasFailure);
	...
	private void buildTests(Element parent, List<Test> tests) {
		...
		for (Test test : sortedTests) {
			...
			topRow.addElement("th").addAttribute("class", "aiotml-test"+testHasFailure).addText(test.getName());
		}
		...
		for (String compiler : compilers) {
			...
			for (Test test : sortedTests) {
				...
				row.addElement("td").addAttribute("class", "aiotml-test"+testHasFailure)
			       .addElement("div").addAttribute("style","background-color:"+res.getResult().toColor()+";color:"+res.getResult().toTextColor())
			   	   ...
			   	   .addAttribute("class", "aiotml-testcase-result"+testCaseHasFailure)
	...
	private void addSummaryChart(Element parent, String name, Summary summary) {
		parent.addElement("canvas").addAttribute("class", "aiotml-summary")
			.addAttribute("data-summary", "{ \"text\": \""+name+"\", \"results\": "+summary.toJSON()+"}");
	}
	
	private void addModal(Element body) {
		Element content = body.addElement("div").addAttribute("class", "modal fade").addAttribute("id", "aiotml-message-modal").addAttribute("tabindex", "-1").addAttribute("role", "dialog")
		...
		content.addElement("div").addAttribute("class", "modal-body")
		       .addElement("pre").addAttribute("class", "aiotml-message panel-danger bg-danger text-danger");
	}
