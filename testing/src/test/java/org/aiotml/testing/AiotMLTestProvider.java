...
public abstract class AiotMLTestProvider implements Describable {
	/* --- Base functions --- */
	
	//public abstract Collection<AiotMLTestCaseCompiler> getCompilers();
	public abstract String[] getCompilers();
	
	//public abstract Collection<AiotMLTestCase> getTestCases();
	public abstract Collection<AiotMLTest> getTests();
	...
	private static class TestFilter {
		...
		public TestFilter() {
			...
			for (String test : tests.split(",")) {
				...
				else if (test.endsWith(".aiotml"))
					files.add(test);
				else
					files.add(test+".aiotml");
			}
		}
	...
	private static final FileFilter testfileFilter = new FileFilter() {
		...
		public boolean accept(File file) {
			return (!file.isDirectory() && file.getName().matches("^test.+\\.aiotml$"));
		}
	};
	
	private void addTestFilesRecursively(TestFilter filter, File dir, Collection<File> files) {		
		// Find all the files with names 'test(.+).aiotml'
