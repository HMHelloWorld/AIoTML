...
public class AiotMLTest implements Describable, Serializable, Callable<Collection<Runnable>> {
	private static final long serialVersionUID = 1L;
	
	protected transient AiotMLModel model;
	...
	protected transient AiotMLTestCase[] cases;
	
	protected boolean shouldFlatten = true;
	
	private transient RunNotifier notifier;
	
	public AiotMLTest(AiotMLModel model, String name, String className, String[] compilers) {
		this.model = model;
		this.name = name;
	...
	private final void generateCases() {
		this.cases = new AiotMLTestCase[this.compilers.length];
		for (int i = 0; i < this.compilers.length; i++) {
			this.cases[i] = AiotMLTestCase.cloneFromId(this.compilers[i], this);
		}
	}
	...
	public boolean prepare(RunNotifier notifier) throws InterruptedException {		
		...
		if (shouldFlatten) {
			synchronized (flattenLock) {
				this.model = AiotMLHelpers.flattenModel(this.model);
			}
		}
		return true;
	}
	...
	public final void run(RunNotifier notifier) {
		...
		for (AiotMLTestCase cse : cases) {
			...
			if (should) {
				...
				try {
					error = cse.run();
				} catch (InterruptedException | AiotMLTimeoutError e) {
