...
public class AiotMLLauncher extends LaunchConfigurationDelegate {
	
	protected File getValidInputFile(ILaunchConfiguration configuration, AiotMLConsole console) throws CoreException {
		...
		if (inputFile == null || !inputFile.exists()) {
			...
			return null;
		} else if (!("aiotml".equals(inputFile.getFileExtension()))) {
			console.printErrorln("Input model is not a .aiotml-file!");
			return null;
		}
	...
	protected AiotMLCompiler getValidCompiler(ILaunchConfiguration configuration, AiotMLConsole console) throws CoreException {
		String selectedCompiler = configuration.getAttribute("org.aiotml.launchconfig.compiler", "");
		if (selectedCompiler.equals(AutoAiotMLCompiler.ID))
	...
	protected File getValidOutdir(ILaunchConfiguration configuration, AiotMLConsole console) throws CoreException {
		String selectedOutdir = configuration.getAttribute("org.aiotml.launchconfig.outdir", "");
	...
	protected Logger getConsoleLogger(AiotMLConsole console) {
		return new Logger() {
			@Override
	...
	protected File getCleanFolderForConfiguration(File outDir, Configuration config, AiotMLConsole console) {
	...
	public void launch(ILaunchConfiguration configuration, String mode, ILaunch launch, IProgressMonitor monitor) throws CoreException {
		AiotMLConsole console = AiotMLConsole.getInstance();
		console.clear();
		console.activate();
		...
		AiotMLCompiler selectedCompiler = getValidCompiler(configuration, console);
		...
		if (!AiotMLCompiler.warnings.isEmpty()) {
			for (String warning : AiotMLCompiler.warnings)
				console.printWarnln(warning);
		}
		
		// Get configurations to compile
		List<Configuration> configs = AiotMLHelpers.allConfigurations(inputModel);
		for (Configuration config : configs) {
			...
			if (configValid) {
				...
				//AiotMLCompiler compiler = selectedCompiler.clone(); // This doesn't currently work as it should
				AiotMLCompiler compiler = registry.createCompilerInstanceByName(selectedCompiler.getID());
		...
		IFile aiotmlFile = (IFile)configuration.getMappedResources()[0]; // We have already checked that this is safe
		aiotmlFile.getProject().refreshLocal(IResource.DEPTH_INFINITE, monitor);
	...
	protected static final AiotMLCompilerRegistry registry = AiotMLCompilerRegistry.getInstance();
	...
	public static class CompilerInfo {
		...
		private static Comparator<CompilerInfo> compareByName() {
			return new Comparator<AiotMLLauncher.CompilerInfo>() {
				@Override
	...
	public static AiotMLCompiler compiler_auto = null;
	static {
		...
		for (String compiler : compilerIDs) {
			if (registry.getCompilerNameById(compiler) != null) {
				if (compiler.equals(AutoAiotMLCompiler.ID)) {
