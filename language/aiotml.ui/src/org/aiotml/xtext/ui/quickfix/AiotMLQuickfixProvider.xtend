 ...
 */
class AiotMLQuickfixProvider extends DefaultQuickfixProvider
 {
	...
	def makeOptional(Issue issue, IssueResolutionAcceptor acceptor) {
		acceptor.accept(
			...
			if (obj instanceof Configuration) {
				...
				val p = AiotMLHelpers.allPorts(i.type).findFirst[p | p instanceof RequiredPort && !(p as RequiredPort).optional && p.name == pName] as RequiredPort
				p.optional = true
			}
		]
	}
	...
	def makeFunctionConcrete(Issue issue, IssueResolutionAcceptor acceptor) {
		acceptor.accept(
			...
			if (obj instanceof Thing) {
				...
				f.abstract = false
				val block = AiotMLFactory.eINSTANCE.createActionBlock
				f.body = block
			}
		]		
	}
	...
	def implementFunction(Issue issue, IssueResolutionAcceptor acceptor) {
		acceptor.accept(
			...
			if (issue.data !== null && obj instanceof Thing) {
				val thing = obj as Thing
				val fName = issue.data.get(0)
				val abs = AiotMLHelpers.allFunctions(thing).findFirst[ f | f.name == fName && f.abstract]
				
				val func = AiotMLFactory.eINSTANCE.createFunction
				func.name = abs.name
				func.abstract = false
				if (abs.typeRef !== null) {
					val typeRef = AiotMLFactory.eINSTANCE.createTypeRef
				...
				abs.parameters.forEach[ p | 
					val param = AiotMLFactory.eINSTANCE.createParameter
					param.name = p.name
					val typeRef = AiotMLFactory.eINSTANCE.createTypeRef
