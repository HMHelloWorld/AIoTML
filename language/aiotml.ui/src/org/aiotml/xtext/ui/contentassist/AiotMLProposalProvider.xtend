 ...
 */
class AiotMLProposalProvider extends AbstractAiotMLProposalProvider {
		...
		def completeActionAndExpression(EObject model, RuleCall ruleCall, ContentAssistContext context, ICompletionProposalAcceptor acceptor) {
			val thing = AiotMLHelpers.findContainingThing(model)
			if (thing != null) {
				AiotMLHelpers.allProperties(thing).forEach[p |
					val String proposal = p.name
					acceptor.accept(createCompletionProposal(proposal, context))
				]
				
				AiotMLHelpers.allFunctions(thing).forEach[f |
					val String proposal = f.name
					acceptor.accept(createCompletionProposal(proposal, context))
				]			
			}
			
			val state = AiotMLHelpers.findContainingState(model)
			if (state != null) {
				AiotMLHelpers.allProperties(state).forEach[p |
					val String proposal = p.name
					acceptor.accept(createCompletionProposal(proposal, context))
				]
			}
		...
		override public void complete_FunctionCallStatement(EObject model, RuleCall ruleCall, ContentAssistContext context, ICompletionProposalAcceptor acceptor) {
			...
			AiotMLHelpers.allFunctions(AiotMLHelpers.findContainingThing(model)).forEach[
				//compute the plain proposal
		...
		override public void complete_SendAction(EObject model, RuleCall ruleCall, ContentAssistContext context, ICompletionProposalAcceptor acceptor) {
			...
			AiotMLHelpers.allPorts(AiotMLHelpers.findContainingThing(model)).forEach[
				val port = it
				//compute the plain proposal				
				it.sends.forEach[
