...
public class ThingsUsage extends AiotMLValidatorCheck {
  ...
  public void checkInstanceUniqueness(final Instance i) {
    ...
    if (_greaterThan) {
      ...
      this.error(msg, cfg, AiotMLPackage.eINSTANCE.getConfiguration_Instances(), cfg.getInstances().indexOf(i), "duplicate-instance");
    }
  }
  
  @Check(CheckType.FAST)
  public void checkPropertyUniqueness(final Property p) {
    final Thing thing = AiotMLHelpers.findContainingThing(p);
    final Function1<Property, Boolean> _function = (Property pr) -> {
      String _name = pr.getName();
      String _name_1 = p.getName();
      return Boolean.valueOf(Objects.equal(_name, _name_1));
    };
    final Iterable<Property> props = IterableExtensions.<Property>filter(AiotMLHelpers.allProperties(thing), _function);
    ...
    if (_greaterThan) {
      ...
      this.error(msg, thing, AiotMLPackage.eINSTANCE.getThing_Properties(), thing.getProperties().indexOf(p), "duplicate-property");
    }
  }
  
  @Check(CheckType.FAST)
  public void checkMessageUniqueness(final Message m) {
    final Thing thing = AiotMLHelpers.findContainingThing(m);
    final Function1<Message, Boolean> _function = (Message m2) -> {
      String _name = m2.getName();
      String _name_1 = m.getName();
      return Boolean.valueOf(Objects.equal(_name, _name_1));
    };
    final Iterable<Message> msgs = IterableExtensions.<Message>filter(AiotMLHelpers.allMessages(thing), _function);
    ...
    if (_greaterThan) {
      ...
      final String msg = (_plus_2 + 
        " is duplicated (possibly from an included thing).");
      this.error(msg, thing, AiotMLPackage.eINSTANCE.getThing_Messages(), thing.getMessages().indexOf(m), "duplicate-message");
    }
  }
  ...
  public void checkPortUniqueness(final Port p) {
    ...
    final Iterable<Port> ports = IterableExtensions.<Port>filter(AiotMLHelpers.allPorts(thing), _function);
    ...
    if (_greaterThan) {
      ...
      final String msg = (_plus_2 + 
        " is duplicated (possibly from an included thing).");
      this.error(msg, thing, AiotMLPackage.eINSTANCE.getThing_Ports(), thing.getPorts().indexOf(p), "duplicate-port");
    }
  }
  ...
  public void checkFunctionUniqueness(final Function f) {
    ...
    final Iterable<Function> funcs = IterableExtensions.<Function>filter(AiotMLHelpers.allFunctions(thing), _function);
    ...
    if (_greaterThan) {
      ...
      final String msg = (_plus_2 + 
        " is duplicated (possibly from an included thing).");
      this.error(msg, thing, AiotMLPackage.eINSTANCE.getThing_Functions(), thing.getFunctions().indexOf(f), "duplicate-function");
    }
  }
}
