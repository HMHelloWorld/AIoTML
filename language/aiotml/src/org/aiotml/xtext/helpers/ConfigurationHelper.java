...
public class ConfigurationHelper {
		...
		for(ConfigPropertyAssign cpa : self.getPropassigns()) {
			if (cpa.getInit() != null && cpa.getInit() instanceof ArrayInit) {
				...
				for(Expression e : ai.getValues()) {
					final ConfigPropertyAssign pa = AiotMLFactory.eINSTANCE.createConfigPropertyAssign();
	...
	public static List<Strategy> allStrategys(Configuration self){
		List<Strategy> result = new ArrayList<Strategy>();
		for(StrategyProperty i : allStrategyPropertys(self)) {
			if (!result.contains(i.getTypeRef().getType()))  
				result.add((Strategy) i.getTypeRef().getType());
		}
		return result;
	}
	public static List<SubThing> allSubThings(Configuration self){
		List<SubThing> result = new ArrayList<SubThing>();
		for(Cproperty i : allCpropertys(self)) {
			if (!result.contains(i.getTypeRef().getType()))  
				result.add((SubThing) i.getTypeRef().getType());
		}
		return result;
	}
	
	public static Set<Cproperty> allCpropertys(Configuration self) {
		Set<Cproperty> result = new HashSet<Cproperty>();
		for(Thing t : allThings(self)) {
			result.addAll(AiotMLHelpers.allCpropertys(t));
		}
		return result;
	}

	public static Set<StrategyProperty> allStrategyPropertys(Configuration self) {
		Set<StrategyProperty> result = new HashSet<StrategyProperty>();
		for(Thing t : allThings(self)) {
			result.addAll(AiotMLHelpers.allStrategyPropertys(t));
		}
		return result;
	}
	...
	public static Set<Message> allMessages(Configuration self) {
		...
		for(Thing t : allThings(self)) {
			result.addAll(AiotMLHelpers.allMessages(t));
		}
		return result;
	}
...
//	public static List<Property> allArrays(SubThing s) {
//		List<Property> result = new ArrayList<Property>();
//		for(Property p : ThingHelper.allPropertiesInDepth(i.getType())) {
//			if (p.getTypeRef().getCardinality() != null || p.getTypeRef().isIsArray())
//				result.add(p);
...
//	public static ArrayList<Property> allSubthingPropertys (EObject container) {
//		ArrayList<Property> result = new ArrayList<Property>();		
//		Configuration conf = findContainer(container,Configuration.class);
//		if (conf != null) {
//		for(SubThing st : allSubThings((Configuration)container)) {
