...
class AiotMLFormatter extends AbstractFormatter2 {
	
	@Inject extension AiotMLGrammarAccess
	...
	def dispatch void format(AiotMLModel aiotMLModel, extension IFormattableDocument document) {
		//println(textRegionAccess.toString())
		// TODO: format HiddenRegions around keywords, attributes, cross references, etc. 
		
		for (Type types : aiotMLModel.getTypes()) {
			types.format;
		}
		for (Protocol protocols : aiotMLModel.getProtocols()) {
			protocols.format;
		}
		for (Configuration configs : aiotMLModel.getConfigs()) {
			configs.format;
		}
		
		//Formats all expressions in the model, so that we do not need to manually find expressions in if, guards, etc
		for(Expression e : AiotMLHelpers.getAllExpressions(aiotMLModel)) {
			e.format
		}
	}	
	...
	def dispatch void format(Action action, extension IFormattableDocument document) {
		/*for(Expression e : AiotMLHelpers.getAllExpressions(action)) {
			e.format
		}*/
