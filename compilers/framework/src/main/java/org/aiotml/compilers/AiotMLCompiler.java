 ...
 */
public abstract class AiotMLCompiler {
    ...
    Map<String, Set<StrategyPlugin>> strategyPluginsPerStrategy = new HashMap<>();
    ...
    public AiotMLCompiler(ThingActionCompiler thingActionCompiler, ThingApiCompiler thingApiCompiler, CfgMainGenerator mainCompiler, CfgBuildCompiler cfgBuildCompiler, ThingImplCompiler thingImplCompiler) {
    ...
    public AiotMLCompiler(NewThingActionCompiler thingActionCompiler, ThingApiCompiler thingApiCompiler, CfgMainGenerator mainCompiler, CfgBuildCompiler cfgBuildCompiler, ThingImplCompiler thingImplCompiler) {
    ...
    public static AiotMLModel loadModel(final File file) { return loadModel(file, Logger.SYSTEM); }
    public static AiotMLModel loadModel(final File file, Logger log) {
        currentFile = file;
        errors = new ArrayList<String>();
        warnings = new ArrayList<String>();

        AiotMLCompiler.registerAiotMLFactory();
        ...
        try {
            ...
            if (errors.isEmpty()) {
                ...
                /*for (Configuration cfg : AiotMLHelpers.allConfigurations(m)) {
        	...
        	log.error("Error loading AiotML model", e);
        }
        return null;
    }
    ...
    private static void registerAiotMLFactory() {
    	AiotMLStandaloneSetup.doSetup();
    }
      

    private static void save(AiotMLModel model, String location) throws IOException {
    ...
    public static void saveAsXMI(final AiotMLModel model, String location) throws IOException {
    	synchronized (saveLock) {
    		AiotMLCompiler.registerXMIFactory();
            AiotMLCompiler.save(model, location);
		}
    }

    public static void saveAsAiotML(final AiotMLModel model, String location) throws IOException {
    	synchronized (saveLock) {
    		AiotMLCompiler.registerAiotMLFactory();
            AiotMLCompiler.save(model, location);
		}
    ...
    public abstract AiotMLCompiler clone();
    ...
    public void addStrategyPlugin(StrategyPlugin sp) {
    	List<String> strategys = sp.getSupportedStrategys();
    	for(String stra:strategys) {
    		if(strategyPluginsPerStrategy.containsKey(stra)) {
    			strategyPluginsPerStrategy.get(stra).add(sp);
    		} else {
    			Set<StrategyPlugin> plugins = new HashSet<>();
    			plugins.add(sp);
    			strategyPluginsPerStrategy.put(stra, plugins);
    		}
    	}
    ...
    public Set<StrategyPlugin> getStrategyPlugins() {
        Set<StrategyPlugin> res = new HashSet<>();
        for (String key : strategyPluginsPerStrategy.keySet()) {
            for (StrategyPlugin sp : strategyPluginsPerStrategy.get(key)) {
                if (!res.contains(sp)) {
                    res.add(sp);
                }
            }
