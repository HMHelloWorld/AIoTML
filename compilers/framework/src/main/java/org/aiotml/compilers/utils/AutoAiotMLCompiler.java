...
public class AutoAiotMLCompiler extends AiotMLCompiler {
	public static final String ID = "auto";
	
	private final Function<String, AiotMLCompiler> getCompiler;
	
	public AutoAiotMLCompiler(Function<String, AiotMLCompiler> getCompilerInstanceByNameFunc) {
		getCompiler = getCompilerInstanceByNameFunc;
	}

	@Override
	public AiotMLCompiler clone() {
		return new AutoAiotMLCompiler(getCompiler);
	}
	...
	public boolean compile(Configuration cfg, Logger log, String... options) {
		...
		AiotMLCompiler actualCompiler = getCompiler.apply(compilerId);
