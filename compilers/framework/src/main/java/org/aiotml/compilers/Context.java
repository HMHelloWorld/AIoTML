...
public class Context {
    ...
    private AiotMLCompiler compiler;
    ...
    public Context(AiotMLCompiler compiler) {
        this.debugStrings = new HashMap<Integer, String>();
        this.compiler = compiler;
    }


    public Context(AiotMLCompiler compiler, String... keywords) {
        this.debugStrings = new HashMap<Integer, String>();
        this.compiler = compiler;
    ...
    public AiotMLCompiler getCompiler() {
        return compiler;
    }
    ...
    public String getTemplateByID(String template_id) {
    	try {
    		...
    		if (this.getCompiler() instanceof OpaqueAiotMLCompiler)
    			((OpaqueAiotMLCompiler)this.getCompiler()).printStack("Template '"+template_id+"' not found.", t);
    		else {
...
//        return getVariableQName(var);
    	return getVariableVarName(var);
    }
    
    public String getVariableQName(Variable var) {
        return AiotMLElementHelper.qname(var, "_") + "_var";
    }
    
    public String getVariableVarName(Variable var) {
        return var.getName()+ "_var";
    }
    ...
    public String traceOnEntry(Thing t, StateContainer r, State s) {
        if (!debugTraceWithID) {
            return " (" + t.getName() + "): Enters " + AiotMLElementHelper.getName(r) + ":" + s.getName();
        } else {
            return null;
        }
    }
    ...
    public String traceOnExit(Thing t, StateContainer r, State s) {
        if (!debugTraceWithID) {
            return " (" + t.getName() + "): Exits " + AiotMLElementHelper.getName(r) + ":" + s.getName();
        } else {
            return null;
        }
    }
    ...
    public void generateStrategyLibs(Configuration cfg) {
    	for (StrategyPlugin sp : this.getCompiler().getStrategyPlugins()) {
            if (!sp.getAssignedStrategys().isEmpty()) {
                sp.generateStrategyLibrary(cfg, this);
            }
        }
    }
    ...
    public SerializationPlugin getSerializationPlugin(Protocol p) throws UnsupportedEncodingException {
        if (AnnotatedElementHelper.hasAnnotation(p, "serializer")) {
            ...
            throw new UnsupportedEncodingException("Protocol " + p.getName() + " has no @serializer annotation. Please provide one in your AiotML file!");
        }
    }
}
