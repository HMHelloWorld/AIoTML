...
public class TestSaveIncludeThingFile extends LoadModelTestsCommon {
	...
	public void test() {
		...
		File test = new File(this.getClass().getResource("/SimpleIncludeModel.aiotml").getFile());
		...
		try {
			...
			AiotMLModel flatmodel = AiotMLHelpers.flattenModel(model);
			
			// Save the AiotML model
			AiotMLCompiler.saveAsAiotML(flatmodel, tmp.toString());
