...
import org.aiotml.compilers.builder.Element;
...
public class GoThingImplCompiler extends ThingImplCompiler {
			...
			for (EventTransitions et : transitions) {
				...
				for (Handler handler : et.guardedHandlers) {
					...
					if (et.msg != null) {
						...
						final String port_name = thing.getName() + "_" + AiotMLHelpers.findContainingThing(receive.getPort()).getName() + "_" + receive.getPort().getName();
						handlerIfBefore
							.append("port == ")
							.append(port_name)
							.append(" && ");
	...
	private void generateInternalPorts(Thing thing, GoSection body, GoContext gctx) {
		for (Port port : AiotMLHelpers.allPorts(thing))
			if (port instanceof InternalPort) {
				final String port_name = thing.getName() + "_" + AiotMLHelpers.findContainingThing(port).getName() + "_" + port.getName();
	...
	public void generateImplementation(Thing thing, Context ctx) {
		...
		if (AiotMLHelpers.allStateMachines(thing).get(0) != null) {
			final CompositeState sm = AiotMLHelpers.allStateMachines(thing).get(0);
		...
		if (AiotMLHelpers.allStateMachines(thing).get(0) != null)
			arguments.append(gctx.getNameFor("state", AiotMLHelpers.allStateMachines(thing).get(0)));
		...
		// Clone properties
		for (Property property : AiotMLHelpers.allProperties(thing)) {
		...
		if (AiotMLHelpers.allStateMachines(thing).get(0) != null) {
			...
			generateSessionForks(thing, AiotMLHelpers.allStateMachines(thing).get(0), builder, gctx);
		}
	}
}
