...
public class GoCompiler extends OpaqueAiotMLCompiler {
	...
	private void generateEnumerations(AiotMLModel model, GoSourceBuilder builder, GoContext gctx) {
		final List<Enumeration> enums = AiotMLHelpers.allEnumerations(model);
	...
	public boolean do_call_compiler(Configuration cfg, Logger log, String... options) {
		...
		generateEnumerations(AiotMLHelpers.findContainingModel(cfg), typesBuilder, ctx);
		...
		for (Message msg : ConfigurationHelper.allMessages(cfg)) {			
			final Thing t = AiotMLHelpers.findContainingThing(msg);
