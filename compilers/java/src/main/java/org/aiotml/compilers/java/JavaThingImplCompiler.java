...
public class JavaThingImplCompiler extends FSMBasedThingImplCompiler {
	...
	public void generateImplementation(Thing thing, Context ctx) {
		...
		boolean hasMessages = AiotMLHelpers.allMessages(thing).size() > 0;
		...
		for (CompositeState b : AiotMLHelpers.allStateMachines(thing)) {
			builder.append("behavior = build" + AiotMLElementHelper.qname(b, "_") + "();\n");
		}
		...
		for (CompositeState b : AiotMLHelpers.allStateMachines(thing)) {
			for (Session s : CompositeStateHelper.allContainedSessions(b)) {
				...
				builder.append("behavior = build" + AiotMLElementHelper.qname(s, "_") + "();\n");
				builder.append("}\n");
			}
		}
	...
	protected void generateCompositeState(StateContainer c, StringBuilder builder, Context ctx) {
		final String state_name = "state_" + AiotMLElementHelper.qname(c, "_");
		for (State s : c.getSubstate()) {
			if (!(s instanceof Session)) {
				if (s instanceof CompositeState) {
					final String s_name = "state_" + AiotMLElementHelper.qname(s, "_");
					builder.append("final CompositeState " + s_name + " = build" + AiotMLElementHelper.qname(s, "_")
							+ "();\n");
				} else {
		...
		if (c instanceof CompositeState) {
			...
			for (Region r : cs.getRegion()) {
				builder.append(state_name + ".add(build" + AiotMLElementHelper.qname(r, "_") + "());\n");
			}
		...
		for (State s : c.getSubstate()) {
			builder.append(state_name + ".add(state_" + AiotMLElementHelper.qname(s, "_") + ");\n");
		}
		if (c.isHistory()) {
			builder.append(state_name + ".keepHistory(true);\n");
		}
		builder.append(state_name + ".initial(state_" + AiotMLElementHelper.qname(c.getInitial(), "_") + ");\n");
	...
	public void generateRegion(StateContainer r, StringBuilder builder, Context ctx) {
		if (r instanceof CompositeState) {
			builder.append("private CompositeState build" + AiotMLElementHelper.qname(r, "_") + "(){\n");
			CompositeState c = (CompositeState) r;
			generateState(c, builder, ctx);
			builder.append("return state_" + AiotMLElementHelper.qname(r, "_") + ";\n");
			...
			builder.append("private Region build" + AiotMLElementHelper.qname(r, "_") + "(){\n");
			buildRegion(r, builder, ctx);
			builder.append("return reg_" + AiotMLElementHelper.qname(r, "_") + ";\n");
		}
		builder.append("}\n\n");
	}

	private void buildRegion(StateContainer r, StringBuilder builder, Context ctx) {
		final String reg_name = "reg_" + AiotMLElementHelper.qname(r, "_");
		...
		for (State s : r.getSubstate()) {
			if (s instanceof CompositeState) {
				builder.append("CompositeState state_" + AiotMLElementHelper.qname(s, "_") + " = build"
						+ AiotMLElementHelper.qname(s, "_") + "();\n");
				builder.append(reg_name + ".add(state_" + AiotMLElementHelper.qname(s, "_") + ");\n");
			} else {
				generateState(s, builder, ctx);
			}
		}
		...
		builder.append(reg_name + ".initial(state_" + AiotMLElementHelper.qname(r.getInitial(), "_") + ");\n");
	}
	...
	private void buildTransition(StringBuilder builder, Context ctx, State s, Handler i, ReceiveMessage msg) {
		...
		//DebugProfile debugProfile = ctx.getCompiler().getDebugProfiles().get(AiotMLHelpers.findContainingThing(s));
		String handler_name = "";
		if (i.getName() != null)
