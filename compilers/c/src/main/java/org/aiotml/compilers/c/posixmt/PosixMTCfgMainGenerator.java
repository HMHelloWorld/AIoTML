 ...
 */
package org.aiotml.compilers.c.posixmt;
...
public class PosixMTCfgMainGenerator extends CCfgMainGenerator {
    ...
    protected void generateConfigurationImplementation(Configuration cfg, AiotMLModel model, PosixMTCompilerContext ctx) {
            ...
            if((!SenderList.isEmpty()) || (!externalSenders.isEmpty())) {
                ...
                for(Map.Entry<Instance, Port> mySender : SenderList.keySet()) {
                    ...
                    for(Map.Entry<Instance, Port> myReceiver : SenderList.get(mySender)) {
                        if (AiotMLHelpers.allStateMachines(myReceiver.getKey().getType()).size() == 0)
                            continue; // there is no state machine

                        
                        CompositeState sm = AiotMLHelpers.allStateMachines(myReceiver.getKey().getType()).get(0);
    ...
    public int generateInstanceInitCode(Instance inst, Configuration cfg, StringBuilder builder, CCompilerContext ctx, int nbConnectorSoFar) {
        ...
        if (AiotMLHelpers.allStateMachines(inst.getType()).size() > 0) { // There is a state machine
            ...
            for(Session s : StateContainerHelper.allContainedSessions(AiotMLHelpers.allStateMachines(inst.getType()).get(0))) {
    ...
    private void generateInitializationCode(Configuration cfg, StringBuilder initb, PosixMTCompilerContext ctx) {
        ...
        while(!Instances.isEmpty()) {
            ...
            if (AiotMLHelpers.allStateMachines(inst.getType()).size() > 0) { // there is a state machine
                ...
                initb.append(AiotMLElementHelper.qname(sm, "_") + "_OnEntry(" + ctx.getStateID(sm) + ", &" + ctx.getInstanceVarName(inst) + ");\n");
            }
