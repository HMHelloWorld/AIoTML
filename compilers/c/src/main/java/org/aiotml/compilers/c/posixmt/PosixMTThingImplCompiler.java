...
public class PosixMTThingImplCompiler extends CThingImplCompiler {
    ...
    private void generateSessionForks(Thing thing, Session s, StringBuilder builder, PosixMTCompilerContext ctx) {
        ...
        builder.append("new_session->s." + ctx.getStateVarName(AiotMLHelpers.allStateMachines(thing).get(0)) + " = " + ctx.getStateID(s.getInitial()) + ";\n");
        ...
        for(Session ss : StateContainerHelper.allContainedSessions(AiotMLHelpers.allStateMachines(thing).get(0))) {
    ...
    private void generateSessionFunctions(Thing thing, StringBuilder builder, PosixMTCompilerContext ctx) {
        for(Session s : CompositeStateHelper.allContainedSessions(AiotMLHelpers.allStateMachines(thing).get(0))) {
            generateSessionForks(thing, s, builder, ctx);
        }
        if(!CompositeStateHelper.allContainedSessions(AiotMLHelpers.allStateMachines(thing).get(0)).isEmpty()) {
    ...
    private void generateMessageProcessQueue(Thing thing, StringBuilder builder, PosixMTCompilerContext ctx) {
        ...
        for (Message m : AiotMLHelpers.allIncomingMessages(thing)) {
    ...
    private void generateThingRun(Thing thing, StringBuilder builder, PosixMTCompilerContext ctx) {
        ...
        if (AiotMLHelpers.allStateMachines(thing).size() > 0) { // there is a state machine
            builder.append("if(_instance->initState != -1) {\n");
            builder.append(AiotMLElementHelper.qname(sm, "_") + "_OnEntry(_instance->initState, _instance);\n");
            builder.append("}\n");
        }
        ...
        if(!StateContainerHelper.allContainedSessions(AiotMLHelpers.allStateMachines(thing).get(0)).isEmpty()) {
    ...
    private void generateMessageEnqueue(Thing thing, StringBuilder builder, PosixMTCompilerContext ctx) {
        ...
        for (Port p : AiotMLHelpers.allPorts(thing)) {
            for (Message m : p.getReceives()) {
                if(StateHelper.canHandleIncludingSessions(AiotMLHelpers.allStateMachines(thing).get(0), p, m)) {
