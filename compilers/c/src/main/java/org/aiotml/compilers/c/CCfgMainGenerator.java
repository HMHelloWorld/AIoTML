 ...
 */
package org.aiotml.compilers.c;
...
import org.aiotml.compilers.NetworkLibraryGenerator;
...
import org.aiotml.xtext.helpers.StateContainerHelper;
...
public class CCfgMainGenerator extends CfgMainGenerator {
    ...
    protected void generateConfigurationImplementation(Configuration cfg, AiotMLModel model, CCompilerContext ctx) {
	...
	"	void * instance;\n" +
        ...
        ctx.getBuilder(ctx.getPrefix() + "aiotml_typedefs.h").append(typedefs_template);
    ...
    protected void generateheaderdeclaration(Configuration cfg, StringBuilder builder, CCompilerContext ctx){
        ...
        builder.append("// Variables for the properties of the instance\n");
            ...
            if (AnnotatedElementHelper.hasAnnotation(cfg, "c_dyn_connectors")) {
                for (Port p : AiotMLHelpers.allPorts(inst.getType())) {
            ...
            CompositeState sm = AiotMLHelpers.allStateMachines(inst.getType()).get(0);
    ...
    protected void generateTypedefs(Configuration cfg, StringBuilder builder, CCompilerContext ctx) {
        ...
        for (Type t : AiotMLHelpers.allUsedSimpleTypes(AiotMLHelpers.findContainingModel(cfg))) {
    ...
    protected void generateMessageHandleCallToDispatcherMessage(Map.Entry<Instance, Port> receiver, Message m,
                                                                StringBuilder builder, CCompilerContext ctx) {
        if (AiotMLHelpers.allStateMachines(receiver.getKey().getType()).size() == 0)
            return; // there is no state machine

        CompositeState sm = AiotMLHelpers.allStateMachines(receiver.getKey().getType()).get(0);
        ...
        for (Message m : ConfigurationHelper.allMessages(cfg)) {
            ...
            if ((!SenderList.isEmpty()) || (!externalSenders.isEmpty())) {
                ...
                for (ExternalConnector eco : externalSenders) {
                    ...
                    CompositeState sm = AiotMLHelpers.allStateMachines(eco.getInst().getType()).get(0);
            ...
            for (Port p : AiotMLHelpers.allPorts(t)) {
                ...
                for (Message m : ConfigurationHelper.allMessageDispatch(cfg, t, p).keySet()) {
                    for (Thing t2 : ConfigurationHelper.allThings(cfg)) {
                        for (Port p2 : AiotMLHelpers.allPorts(t2)) {
    ...
    protected void generateCfgInitializationCode(Configuration cfg, StringBuilder builder, StringBuilder headerbuilder, CCompilerContext ctx) {
        ...
        for (Thing t : ConfigurationHelper.allThings(cfg)) {
            for (Port port : AiotMLHelpers.allPorts(t)) {
    ...
    public int generateInstanceInitCode(Instance inst, Configuration cfg, StringBuilder builder, CCompilerContext ctx, int nbConnectorSoFar) {
        ...
        builder.append("add_instance( (void*) &" + ctx.getInstanceVarName(inst) + ");\n");
            ...
            int i = 0;
                ...
                //i.e. when the event is taken into account in the sm
                    ...
                    if (AiotMLHelpers.allStateMachines(inst.getType()) != null) {
                        if (StateHelper.allMessageHandlers(AiotMLHelpers.allStateMachines(inst.getType()).get(0)) != null) {
                            if (StateHelper.allMessageHandlers(AiotMLHelpers.allStateMachines(inst.getType()).get(0)).get(p) != null) {
                                if (StateHelper.allMessageHandlers(AiotMLHelpers.allStateMachines(inst.getType()).get(0)).get(p).containsKey(m)) {
        ...
        if (AiotMLHelpers.allStateMachines(inst.getType()).size() > 0) { // There is a state machine
            for (StateContainer r : CompositeStateHelper.allContainedStateContainers(AiotMLHelpers.allStateMachines(inst.getType()).get(0))) {
                builder.append(ctx.getInstanceVarName(inst) + "." + ctx.getStateVarName(r) + " = " + ctx.getStateID(r.getInitial()) + ";\n");
            }
            for(Session s : CompositeStateHelper.allContainedSessions(AiotMLHelpers.allStateMachines(inst.getType()).get(0))) {
    ...
    public void generateInstanceOnEntryCode(Instance inst, StringBuilder builder, CCompilerContext ctx) {
        if (AiotMLHelpers.allStateMachines(inst.getType()).size() > 0) { // there is a state machine
            CompositeState sm = AiotMLHelpers.allStateMachines(inst.getType()).get(0);
            builder.append(AiotMLElementHelper.qname(sm, "_") + "_OnEntry(" + ctx.getStateID(sm) + ", &" + ctx.getInstanceVarName(inst) + ");\n");
        }
    }
    ...
    protected void generatePollingCode(Configuration cfg, StringBuilder builder, CCompilerContext ctx) {
        ...
        for (Instance i : ConfigurationHelper.allInstances(cfg)) {
            ...
            if (AiotMLHelpers.allStateMachines(i.getType()).size() > 0) { // There has to be only 1
                CompositeState sm = AiotMLHelpers.allStateMachines(i.getType()).get(0);
        ...
        for (Instance inst : ConfigurationHelper.allInstances(cfg)) {
        	for (Function f : AiotMLHelpers.allFunctions(inst.getType())) {
