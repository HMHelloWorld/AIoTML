...
import org.aiotml.compilers.interfaces.c.ICThingImpEventHandlerStrategy;
...
public class CThingImplCompiler extends FSMBasedThingImplCompiler {
    ...
    protected void generateStateMachineOnExitCPrototypes(Thing thing, StringBuilder builder, CCompilerContext ctx) {
        if (AiotMLHelpers.allStateMachines(thing).size() > 0) {// There should be only one if there is one
            CompositeState sm = AiotMLHelpers.allStateMachines(thing).get(0);
            builder.append("void " + AiotMLElementHelper.qname(sm, "_") + "_OnExit(int state, ");
    ...
    protected void generatePrivateCPrototypes(Thing thing, StringBuilder builder, CCompilerContext ctx) {
        ...
        for (Port port : AiotMLHelpers.allPorts(thing)) {
    ...
    protected void generateEntryActions(Thing thing, StringBuilder builder, CCompilerContext ctx) {
        ...
        if (AiotMLHelpers.allStateMachines(thing).isEmpty()) return;

        StringBuilder cppHeaderBuilder = ctx.getCppHeaderCode();

        CompositeState sm = AiotMLHelpers.allStateMachines(thing).get(0); // There has to be one and only one state machine here

        builder.append("void " + getCppNameScope() + AiotMLElementHelper.qname(sm, "_") + "_OnEntry(int state, ");
        builder.append("struct " + ctx.getInstanceStructName(thing) + " *" + ctx.getInstanceVarName() + ") {\n");
        ...
        for (CompositeState cs : CompositeStateHelper.allContainedCompositeStatesIncludingSessions(sm)) {
            ...
            for (StateContainer r : regions) {
                builder.append(AiotMLElementHelper.qname(sm, "_") + "_OnEntry(" + ctx.getInstanceVarName() + "->" + ctx.getStateVarName(r) + ", " + ctx.getInstanceVarName() + ");\n");
            }
    ...
    protected void generateExitActions(Thing thing, StringBuilder builder, CCompilerContext ctx) {
        ...
        builder.append("void " + getCppNameScope() + AiotMLElementHelper.qname(sm, "_") + "_OnExit(int state, ");
        builder.append("struct " + ctx.getInstanceStructName(thing) + " *" + ctx.getInstanceVarName() + ") {\n");
        builder.append("switch(state) {\n");
        ...
        for (CompositeState cs : CompositeStateHelper.allContainedCompositeStatesIncludingSessions(sm)) {
            ...
            // Init state
            for (StateContainer r : regions) {
                builder.append(AiotMLElementHelper.qname(sm, "_") + "_OnExit(" + ctx.getInstanceVarName() + "->" + ctx.getStateVarName(r) + ", " + ctx.getInstanceVarName() + ");\n");
            }
    ...
    public void generateEmptyHandlers(Thing thing, State s, StringBuilder builder, CompositeState cs, StateContainer r, CCompilerContext ctx) {
        ...
        for (Handler h : StateHelper.emptyHandlers(s)) {
            ...
            if (h instanceof InternalTransition) {
                ...
                builder.append(AiotMLElementHelper.qname(AiotMLHelpers.allStateMachines(thing).get(0), "_") + "_OnExit(" + ctx.getStateID((State)et.eContainer()) + ", " + ctx.getInstanceVarName() + ");\n");
                ...
                builder.append(AiotMLElementHelper.qname(AiotMLHelpers.allStateMachines(thing).get(0), "_") + "_OnEntry(" + ctx.getStateID(et.getTarget()) + ", " + ctx.getInstanceVarName() + ");\n");
    ...
    protected void generateMessageHandlers(Thing thing, State s, Port port, Message msg, StringBuilder builder, CompositeState cs, StateContainer r, CCompilerContext ctx) {
        ...
        for (ReceiveMessage mh : events) {
            ...
            Handler h = AiotMLElementHelper.findContainingHandler(mh);
    ...
    private void generateSessionTerminate(Thing thing, StringBuilder builder, CCompilerContext ctx) {
        if(StateContainerHelper.allContainedSessions(AiotMLHelpers.allStateMachines(thing).get(0)).size() > 0) {
            builder.append("void " + thing.getName() + "_terminate(struct " + ctx.getInstanceStructName(thing) + " * _instance) {\n");
            for(Session s : StateContainerHelper.allContainedSessions(AiotMLHelpers.allStateMachines(thing).get(0))) {
                builder.append("    _instance->active = false;\n");
    ...
    private void generateSessionForks(Thing thing, StringBuilder builder, CCompilerContext ctx) {
        for(Session s : StateContainerHelper.allContainedSessions(AiotMLHelpers.allStateMachines(thing).get(0))) {
            ...
            builder.append("    new_session->" + ctx.getStateVarName(AiotMLHelpers.allStateMachines(thing).get(0)) + " = " + ctx.getStateID(s.getInitial()) + ";\n");
            CompositeState sm = AiotMLHelpers.allStateMachines(thing).get(0);
