...
import org.aiotml.compilers.interfaces.c.ICThingApiIncludesStrategy;
import org.aiotml.compilers.interfaces.c.ICThingApiPublicPrototypeStrategy;
import org.aiotml.compilers.interfaces.c.ICThingApiStateIDStrategy;
import org.aiotml.compilers.interfaces.c.ICThingApiStructStrategy;
...
public class CThingApiCompiler extends ThingApiCompiler {
    ...
    protected void generateCHeaderAnnotation(Thing thing, StringBuilder builder, CCompilerContext ctx) {
        ...
        for (Thing t : AiotMLHelpers.allThingFragments(thing)) {
    ...
    protected void generateInstanceStruct(Thing thing, StringBuilder builder, CCompilerContext ctx) {
        ...
        if (AiotMLHelpers.allStateMachines(thing).size() > 1) {
            throw new Error("Info: Thing " + thing.getName() + " has " + AiotMLHelpers.allStateMachines(thing).size() + " state machines. " + "Error: Code generation for Things with several state machines not implemented.");
        }
    ...
    protected void generatePublicPrototypes(Thing thing, StringBuilder builder, CCompilerContext ctx) {
        ...
        if (AiotMLHelpers.allStateMachines(thing).size() > 0) {// There should be only one if there is one
            CompositeState sm = AiotMLHelpers.allStateMachines(thing).get(0); // There should be one and only one
            // Entry actions
            builder.append("void " + AiotMLElementHelper.qname(sm, "_") + "_OnEntry(int state, ");
