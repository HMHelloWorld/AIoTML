...
import org.aiotml.compilers.javascript.JSSourceBuilder.StateJSState;
import org.aiotml.compilers.javascript.JSSourceBuilder.StateJSTransition;
...
public abstract class JSThingImplCompiler extends NewFSMBasedThingImplCompiler {
	...
	protected void generateStateMachine(CompositeState sm, Section section, Context ctx) {
		...
		initial.assignTo("let _initial_" + AiotMLElementHelper.qname(sm, "_"));
		...
		// Handlers
		section.append("_initial_" + AiotMLElementHelper.qname(sm, "_") + ".to(" + AiotMLElementHelper.qname(sm.getInitial(), "_") + ");");
	...
	protected void generateCompositeState(StateContainer cs, Section section, Context ctx) {
		if (cs instanceof Session) {
			...
			composite.assignTo("let " + AiotMLElementHelper.qname(cs, "_"));
			...
			if (cs instanceof CompositeState && CompositeStateHelper.hasSeveralRegions((CompositeState)cs)) {
				...
				for (Region r : ((CompositeState)cs).getRegion()) {
					ctx.addContextAnnotation("container", AiotMLElementHelper.qname(cs, "_"));
					generateRegion(r, section, ctx);
				}
				for (State s : cs.getSubstate()) {
					ctx.addContextAnnotation("container", AiotMLElementHelper.qname(cs, "_")/* + "_default"*/);
					generateState(s, section, ctx);
				}
			} else {
			...
			StateJSState initial = JSSourceBuilder.stateJSState(section, "_initial", "PseudoState");
			initial.assignTo("let _initial_" + AiotMLElementHelper.qname(cs, "_"));
			initial.setParent(AiotMLElementHelper.qname(cs, "_"));
			if (cs.isHistory()) initial.setPseudoStateKind("ShallowHistory");
			else initial.setPseudoStateKind("Initial");
			
			// Handlers
			section.append("_initial_" + AiotMLElementHelper.qname(cs, "_") + ".to(" + AiotMLElementHelper.qname(cs.getInitial(), "_") + ");");
		}
	}
	...
	protected void generateAtomicState(State s, Section section, Context ctx) {
		...
		state.assignTo("let " + AiotMLElementHelper.qname(s, "_"));
	...
	public void generateRegion(StateContainer r, Section section, Context ctx) {
		...
		region.assignTo("let " + AiotMLElementHelper.qname(r, "_") + "_reg");
		region.setParent(containerName);
		
		StateJSState initial = JSSourceBuilder.stateJSState(section, "_initial", "PseudoState");
		initial.assignTo("let _initial_" + AiotMLElementHelper.qname(r, "_") + "_reg");
		initial.setParent(AiotMLElementHelper.qname(r, "_")+"_reg");
		if (r.isHistory()) initial.setPseudoStateKind("ShallowHistory");
		else initial.setPseudoStateKind("Initial");
		
		for (State s : r.getSubstate()) {
			ctx.addContextAnnotation("container", AiotMLElementHelper.qname(r, "_") + "_reg");
			generateState(s, section, ctx);
		}
		
		// Handlers
		section.append("_initial_" + AiotMLElementHelper.qname(r, "_") + "_reg.to(" + AiotMLElementHelper.qname(r.getInitial(), "_") + ");");
	}
	...
	protected void generateTransition(Transition t, Message msg, Port p, Section section, Context ctx) {
		...
		if (p != null && msg != null) {//empty transition
			transition = JSSourceBuilder.stateJSTransition(section, "Event."+ctx.firstToUpper(msg.getName()), AiotMLElementHelper.qname((State)t.eContainer(), "_"), AiotMLElementHelper.qname(t.getTarget(), "_"));
		} else {
			transition = JSSourceBuilder.stateJSTransition(section, null, AiotMLElementHelper.qname((State)t.eContainer(), "_"), AiotMLElementHelper.qname(t.getTarget(), "_"));
		}
		
		transition.setTo(AiotMLElementHelper.qname(t.getTarget(), "_"));
	...
	protected void generateInternalTransition(InternalTransition t, Message msg, Port p, Section section, Context ctx) {
		String containerName = AiotMLElementHelper.qname((State)t.eContainer(), "_");
